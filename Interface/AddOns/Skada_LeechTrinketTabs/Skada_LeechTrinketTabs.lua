...
local leechedmod = Skada:NewModule("Trinket Leech players")
...
local leechTaken = Skada:NewModule("Trinket Leech taken")
...
local function spell_tooltip(win, id, label, tooltip)
	...
	if player then
		if(label ~= GetSpellInfo(184671)) then
...
-- ##### mod: Leech trinket done by each player ##### --
...
-- ##### leechedmod: Leech trinket done by a player distributed by target ##### --
function leechedmod:Enter(win, id, label)
	leechedmod.playerid = id
	leechedmod.title = "Leeched by " .. label
end

-- ##### leechTaken: Leech trinket recived by each player ##### --
local httable = {}
...
-- ##### fullleech: Normal healing + Leech trinket done by each player ##### --
...
-- ##### spellsmod: Normal healing + Leech trinket done by a player distributed by spell ##### --
...
function spellsmod:Update(win, set)
	...
	if player then
		...
		if player.leech > 0 then
			local d = win.dataset[nr] or {}
			win.dataset[nr] = d
			local name = "Shadowfel Infusion"
			_, _, icon = GetSpellInfo(41220)
...
-- ##### healedmod: Normal healing + Leech trinket done by a player distributed by target ##### --
