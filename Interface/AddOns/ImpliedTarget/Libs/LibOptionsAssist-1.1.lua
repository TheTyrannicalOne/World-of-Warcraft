local LIB, REVISION = "LibOptionsAssist-1.1", 4
...
local AddChild
do
	local info = { }			-- info is doubling as 'feedpath'

	local function OnGroupSelected(self, event, value)
		local data = self:GetUserDataTable()
		local option, options, path = data.option, data.options, data.path
		...
		if type(option.onTabSelect) == 'function' then
			info[0], info.appName = data.appName, data.appName
			info.option, info.options = option, options
			info.arg, info.type = option.arg, option.type
			pcall(option.onTabSelect, info, value)
		end

		info[#info + 1] = value
		ACD:FeedGroup(data.appName, options, self, data.rootframe, info)
	...
	function AddChild(self, child, ...)
		...
		if child.type ~= 'TabGroup' then return end
		child:SetCallback('OnGroupSelected', OnGroupSelected)
		local selected = child.status and child.status.selected
		if selected then
			OnGroupSelected(child, 'OnGroupSelected', selected)
		end
	end
end
...
local function Open(entry, toggle)
	if entry:IsVisible() then
		...
		InterfaceOptionsFrame_OpenToCategory(entry)
		if not entry:IsVisible() then											-- Temp fix for Blizzard bug
			InterfaceOptionsFrame_OpenToCategory(entry)
		end
	end
	return entry
...
local processLOD
do
	local function process_pcall_result(ok, ...)
		if ok then
			if select('#', ...) > 1 then
				return true, { ... }
			else
	...
	processLOD = {
		['function'] = function(func, entry)
			local ok, result = process_pcall_result(pcall(func, entry))
			if ok then
				return result
			end
		...
		['string'] = function(name)
			LOD.caller = name
			local loaded, reason = LoadModule(name)
			LOD.caller = nil
			if not loaded then
				...
				geterrorhandler()(ADDON_LOAD_FAILED:format(name, reason))
			end
		end,
		['table'] = function(data)
			LOD.payload = data
	...
	['Show'] = function(self)													-- Required
		if isValid[self] then
			...
			if LOD[self] then													-- Do here to avoid potential LoD issues
				...
				while lod do
					local func = processLOD[type(lod)]
					lod = func and func(lod, self)
				end
				LOD.payload = nil												-- May have been set in while loop
			end
			showing = self
			frame:Show()
		end
...
function lib.AddEntry(name, parent, loadOnDemand, key)
	...
	elseif loadOnDemand and not processLOD[type(loadOnDemand)] then
		error(ERROR_TYPE:format(3, "AddEntry", "function, table, string, or nil", type(loadOnDemand)), 2)
	elseif parent then
	...
	elseif loadOnDemand == "parent" then
		error(ERROR_BASE:format(3, "AddEntry", ("only child entries may use %q"):format("parent")), 2)
	end
...
function lib.FetchData()
	if LOD.payload then
		local _, path = pcall(error, "", 3)
		local _, _, name = strsplit("\\", path)
		if LOD.caller == name then
			return select(2, unpack(LOD.payload))
		end
	end
end
...
function lib.OpenEntry(name, parent, toggle)
	if type(name) ~= 'string' then
		error(ERROR_TYPE:format(1, "OpenEntry", 'string', type(name)), 2)
	elseif parent and type(parent) ~= 'string' then
		error(ERROR_TYPE:format(2, "OpenEntry", "string or nil", type(parent)), 2)
	end
	local index, entry = FindEntry(name, parent)
	if entry then
		return Open(entry, toggle)
	end
end

function lib.OpenNextMember(name, resume)
	if type(name) ~= 'string' then
		error(ERROR_TYPE:format(1, "OpenNextMember", 'string', type(name)), 2)
	end
	local index, entry = FindEntry(name)
	if not entry then return end
	if entry.hasChildren then
		local startIndex, stopIndex = index, index
		for index = index + 1, #entries do
			if not entries[index] or entries[index].parent ~= name then
				stopIndex = index - 1
				break
			end
		end
		for index = startIndex, stopIndex do
			if entries[index]:IsShown() then
				entry = entries[entries[index]:IsVisible() and (index < stopIndex and index + 1 or startIndex) or (resume and index or startIndex)]
				break
			end
		end
	end
...
function lib.SortChildren(parent, sortFunc)
	...
	local numChildren = 0
	for _, entry in iterate_children, parent, index do
	...
	for offset = 1, numChildren do
		entries[index + offset] = order[offset]
	end
	if InterfaceOptionsFrameAddOns:IsShown() then
		InterfaceAddOnsList_Update()
	end
