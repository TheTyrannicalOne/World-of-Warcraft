...
--local font = 'Interface\\AddOns\\AlhanaUI\\Core\\Fonts\\agencyb.ttf'
...
function Update(self)
	...
	for index, value in ipairs(active[id]) do
		...
		if (not bar) then
			...
			if (self.Mode == "ICON") then
				...
				if (bar.count) then
					...
					bar.count:SetFont(font, 12, "MONOCHROMEOUTLINE")
			...
			Movers:RegisterFrame(bar)	
				...
				if (bar.background)then
					...
					bar.background:SetTexture(C["Medias"].Normal)
				...
				if (bar.time)then
					...
					bar.time:SetFont(font, 12, "MONOCHROMEOUTLINE")
				...
				if (bar.spellname)then
					...
					bar.spellname:SetFont(font, 12, "MONOCHROMEOUTLINE")
		...
		if (value.duration > 0) then
			if (self.Mode == "ICON") then
				CooldownFrame_Set(bar.cooldown, value.data.filter == "CD" and value.expirationTime or (value.expirationTime - value.duration), value.duration, 1)
...
local function OnEvent(self, event, ...)
	...
	if ((unit == "target" or unit == "player" or unit == "pet" or unit == "focus") or event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_ENTERING_WORLD" or event == "SPELL_UPDATE_COOLDOWN") then
		...
		for i=1, #Filger_Spells[class][id], 1 do
			local data, name, icon, count, debuffType, duration, expirationTime, caster, isStealable, start, enabled, slotLink, spn
			data = Filger_Spells[class][id][i]
			if (data.filter == "BUFF") then
				spn = GetSpellInfo(data.spellID)
				name, icon, count, debuffType, duration, expirationTime, caster, isStealable = UnitBuff(data.unitId, spn)
				icon = GetSpellTexture(data.spellID)
			elseif (data.filter == "DEBUFF") then
				spn = GetSpellInfo(data.spellID)
				name, icon, count, debuffType, duration, expirationTime, caster, isStealable = UnitDebuff(data.unitId, spn)
				icon = GetSpellTexture(data.spellID)
			else
	...
	for i = 1, #Filger_Spells[class], 1 do
		...
		if (f_s.configmode) then
			...
			for j = 1, #Filger_Spells[class][i], 1 do
				...
				if (data.spellID) then
					spellIcon = GetSpellTexture(data.spellID)
				else
...
hooksecurefunc(CD, "Enable", Enable)