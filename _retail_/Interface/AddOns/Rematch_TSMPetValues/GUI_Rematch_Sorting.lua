...
local Rematch_Sorting = addon:NewModule("Rematch_Sorting", "AceConsole-3.0")
...
function Rematch_Sorting:DebugPrintf(...)
...
function Rematch_Sorting:ModuleName()
	return Rematch_Sorting.moduleName
end

function Rematch_Sorting:Login()
	Rematch_Sorting:DebugPrintf("Login()")

	addon.db.global[Rematch_Sorting:ModuleName()] = false
	if Rematch and Rematch.Roster.SortPets and Rematch.GetIDType then
		Rematch_Sorting:DebugPrintf("   found Rematch and hook into Rematch.Roster.SortPets()")

		private.OldRRSP = Rematch.Roster.SortPets
		Rematch.Roster.SortPets = addon.NewRRSP

		addon.db.global[Rematch_Sorting:ModuleName()] = true
	end
end

-- itemString, customName, name = GetPetItemString(petID)
local function GetPetItemString(petID)
	Rematch_Sorting:DebugPrintf("GetPetItemString(%s)", tostring(petID))
...
function addon.NewRRSP(pet1, pet2)
	-- Rematch_Sorting:DebugPrintf("NewRRSP(%s, %s)", tostring(pet1), tostring(pet2))
	local doSort
	...
	then
		...
		return private.OldRRSP(pet1, pet2)
	end
	...
	if doSort then
		...
		if petString1 and petString2 then
			...
			if RematchSettings.Sort.Order == private.sortValA then
				p1 = addon.PetItem and addon.PetItem.GetPetGoldValue(petString1, addon.db.global.valA) or 0
				p2 = addon.PetItem and addon.PetItem.GetPetGoldValue(petString2, addon.db.global.valA) or 0
			end
			if RematchSettings.Sort.Order == private.sortValB then
				p1 = addon.PetItem and addon.PetItem.GetPetGoldValue(petString1, addon.db.global.valB) or 0
				p2 = addon.PetItem and addon.PetItem.GetPetGoldValue(petString2, addon.db.global.valB) or 0
			end
			if RematchSettings.Sort.Order == private.sortValW then
				local p1a = addon.PetItem and addon.PetItem.GetPetGoldValue(petString1, addon.db.global.valA) or 0
				local p1b = addon.PetItem and addon.PetItem.GetPetGoldValue(petString1, addon.db.global.valB) or 0
				local p2a = addon.PetItem and addon.PetItem.GetPetGoldValue(petString2, addon.db.global.valA) or 0
				local p2b = addon.PetItem and addon.PetItem.GetPetGoldValue(petString2, addon.db.global.valB) or 0
				if p1a and p1b then
					p1 = p1a - p1b
				end
				if p2a and p2b then
...
function Rematch_Sorting:SecTimer()
	if secTimerDone then
		return
	end

	Rematch_Sorting:DebugPrintf("SecTimer()")
	...
	if Rematch and Rematch.GetMenu and Rematch.Roster and Rematch.Roster.GetSort and Rematch.Roster.SetSort then
		Rematch_Sorting:DebugPrintf("   found Rematch PetSort Menu and extending it...")
