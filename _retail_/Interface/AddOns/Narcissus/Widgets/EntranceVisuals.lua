...
local function Entrance_DH(self, elapsed)
	...
	local turnTime = 0.1
	local t = 0.25;
	local offset = outQuad(self.TimeSinceLastUpdate, 2, self.defaultZ - 2, t)
...
local function Entrance_Monk(self, elapsed)
	...
	local turnTime = 0.36
	local t = 1;
	local offset = outQuad(self.TimeSinceLastUpdate, self.startY, self.defaultY - self.startY, t)
...
local function Entrance_Warrior(self, elapsed)
	...
	local turnTime = 0.1
	local t = 0.25;
	local offset = outQuad(self.TimeSinceLastUpdate, 6, self.defaultZ - 6, t)
